...
-- postagePrice w = if w <= 500 then 250 else if w <= 5000 then 300 + w else 6000
...
-- sumTo n = if n <= 1 then 1 else sumTo(n - 1) + n
sumTo n
  | n <= 1 = 1
...
roundDown n d = n `div` d

ilogHelper n acc 
    | n <= 0 = acc
    | otherwise = ilogHelper (roundDown n 3) (acc + 1)

ilog3 :: Integer -> Integer
ilog3 n = ilogHelper n 0
