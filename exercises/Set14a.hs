...
greetText name = if T.length name <= 15
    then T.pack "Hello, " <> name <> T.pack "!"
    else T.pack "Hello, " <> T.take 15 name <> T.pack "...!"
...
shout :: T.Text -> T.Text
shout text =
    let zipped = zip [0..] (T.words text)
        mapped = map (\(i, word) -> if even i then T.toUpper word else word) zipped
    in T.unwords mapped
...
longestRepeat t = if T.null t then 0 else
    let
        grouped = T.groupBy (==) t
        lengths = map T.length grouped
        in maximum lengths
------------------------------------------------------------------------------
...
pingpong b =
    BL.cycle (BL.fromStrict b <> BL.fromStrict (B.reverse b))
