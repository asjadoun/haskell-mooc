...
buyOneMore (MkShoppingEntry "Banana" p c) = MkShoppingEntry "Banana" p (c+1)
buyOneMore (MkShoppingEntry "Apple" p c) = MkShoppingEntry "Apple" p (c+1)
buyOneMore (MkShoppingEntry item p c) = MkShoppingEntry item p (c+1)
------------------------------------------------------------------------------
...
tick :: UpDown -> UpDown
tick (Increasing c) = Increasing (c+1)
tick (Decreasing c) = Decreasing (c-1)


-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (Increasing c) = Decreasing c
toggle (Decreasing c) = Increasing c
...
toList Empty = []
toList (Pair x y kv) = (x,y) : toList kv

fromList :: [(k,v)] -> KeyVals k v
fromList [] = Empty
fromList ((x,y):zs) = Pair x y (fromList zs)
