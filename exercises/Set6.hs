...
instance Ord Country where
  compare c1 c2 = compare (country c1) (country c2)
...
instance Ord Number where
  ...
  compare (Finite n1) (Finite n2) = compare n1 n2
...
mygcd a b = if b==0 then a else gcd b (mod a b)

simplify :: RationalNumber -> RationalNumber
simplify (RationalNumber a b) =
  let x = mygcd a b
  in RationalNumber (div a x) (div b x)
...
instance Num RationalNumber where
  ...
  abs p@(RationalNumber a b)
    | a < 0 && b < 0 = simplify (RationalNumber (- (1 * a)) (- (1 * b)))
    | a < 0 = simplify (RationalNumber (- (1 * a)) b)
    | b < 0 = simplify (RationalNumber a (- (1 * b)))
    | otherwise = p
  signum p@(RationalNumber a b)
    | a == 0 || b == 0 = RationalNumber 0 1
    | a < 0 || b < 0 = RationalNumber (-1) 1
    | otherwise = RationalNumber 1 1
  fromInteger x = RationalNumber x 1
  negate (RationalNumber a b) = simplify (RationalNumber (-(1*a)) b)
